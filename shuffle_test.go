...
var specTests = []data{
	...
	{"Chained Before", "AB<C<D<E<", "ABCDE|BCDEA"},
	{"Chained Before", "AB<CD<E", "ABCDE|ABECD|CDABE|CDEAB|EABCD|ECDAB"},
}

// Shuffle based on specification string.
func shuffleSpec(in string, shuf *Shuffle) (string, error) {
...
// Ensure that shuffle is not confusing indices with items.
...
func TestIndexItem(t *testing.T) {
	...
	for _, j := range out {
        ...
        if seen[j] {
          t.Errorf("Shuffle returned duplicate index: %d\n", j)
        } else {
          seen[j] = true
        }
	}
